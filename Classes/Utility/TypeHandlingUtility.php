 ...
 */
class Tx_Extbase_Utility_TypeHandlingUtility {
	 ...
	 * @throws Tx_Extbase_Utility_Exception_InvalidTypeException
	 */
	static public function parseType($type) {
		...
		if (preg_match(self::PARSE_TYPE_PATTERN, $type, $matches)) {
			...
			if ($elementType !== NULL && !self::isCollectionType($type)) {
				throw new Tx_Extbase_Utility_Exception_InvalidTypeException('Found an invalid element type declaration in %s. Type "' . $type . '" must not have an element type hint (' . $elementType . ').', 1264093642);
			}
			...
			throw new Tx_Extbase_Utility_Exception_InvalidTypeException('Found an invalid element type declaration in %s. A type "' . var_export($type, TRUE) . '" does not exist.', 1264093630);
		}
	}
