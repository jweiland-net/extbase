 ...
 */
class Tx_Extbase_MVC_Controller_MvcPropertyMappingConfigurationService implements t3lib_Singleton {
		...
		foreach ($formFieldNames as $formField) {
			...
			for ($i = 0; $i < count($formFieldParts); $i++) {
				...
				if ($i === count($formFieldParts) - 1) {
					...
					if ($formFieldPart === '') {
						throw new Tx_Extbase_Security_Exception_InvalidArgumentForHashGeneration('The form field "' . $formField . '" is invalid. Reason: "[]" used not as last argument, but somewhere in the middle (like foo[][bar]).', 1255072832);
					}
	 ...
	 * @param Tx_Extbase_MVC_Controller_Arguments $controllerArguments
	 *
	 * @return void
	 */
	public function initializePropertyMappingConfigurationFromRequest(Tx_Extbase_MVC_Request $request, Tx_Extbase_MVC_Controller_Arguments $controllerArguments) {
	 ...
	 * @param Tx_Extbase_Property_PropertyMappingConfiguration $propertyMappingConfiguration
	 *
	 * @return void
	 */
	protected function modifyPropertyMappingConfiguration($propertyConfiguration, Tx_Extbase_Property_PropertyMappingConfiguration $propertyMappingConfiguration) {
		if (!is_array($propertyConfiguration)) {
			return;
		}

		if (isset($propertyConfiguration['__identity'])) {
			$propertyMappingConfiguration->setTypeConverterOption('Tx_Extbase_Property_TypeConverter_PersistentObjectConverter', Tx_Extbase_Property_TypeConverter_PersistentObjectConverter::CONFIGURATION_MODIFICATION_ALLOWED, TRUE);
			unset($propertyConfiguration['__identity']);
		} else {
			$propertyMappingConfiguration->setTypeConverterOption('Tx_Extbase_Property_TypeConverter_PersistentObjectConverter', Tx_Extbase_Property_TypeConverter_PersistentObjectConverter::CONFIGURATION_CREATION_ALLOWED, TRUE);
		}
