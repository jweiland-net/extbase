...
class Tx_Extbase_Property_PropertyMapper implements t3lib_Singleton {
	 ...
	 *
	 * @return void
	 * @throws Tx_Extbase_Property_Exception_DuplicateTypeConverterException
	...
	protected function doMapping($source, $targetType, Tx_Extbase_Property_PropertyMappingConfigurationInterface $configuration, &$currentPropertyPath) {
		...
		foreach ($typeConverter->getSourceChildPropertiesToBeConverted($source) as $sourcePropertyName => $sourcePropertyValue) {
			...
			if (!$configuration->shouldMap($targetPropertyName)) {
				...
				throw new Tx_Extbase_Property_Exception_InvalidPropertyMappingConfigurationException('It is not allowed to map property "' . $targetPropertyName . '". You need to use $propertyMappingConfiguration->allowProperties(\'' . $targetPropertyName . '\') to enable mapping of this property.', 1355155913);
			}
			...
			if (!($targetPropertyValue instanceof Tx_Extbase_Error_Error)) {
	...
	protected function findTypeConverter($source, $targetType, Tx_Extbase_Property_PropertyMappingConfigurationInterface $configuration) {
		...
		if (Tx_Extbase_Utility_TypeHandlingUtility::isSimpleType($targetType)) {
